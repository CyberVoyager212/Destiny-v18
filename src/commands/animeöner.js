...
exports.execute = async (client, message, args) => {
  ...
  if (args[0] && args[0].toLowerCase() === 'sil') {
    ...
    if (channelToDelete) {
      await channelToDelete
        .delete('Anime öneri sistemi yönetici tarafından silindi.')
        .catch(() => null);
    }
  ...
  if (!animeChannelId || !message.guild.channels.cache.has(animeChannelId)) {
    const row = new MessageActionRow().addComponents(
      new MessageButton()
        .setCustomId('setup_anime_yes')
        .setLabel('Evet, Kur')
        .setStyle('SUCCESS'),
      new MessageButton()
        .setCustomId('setup_anime_no')
        .setLabel('İptal Et')
        .setStyle('DANGER'),
    );
    ...
    const questionEmbed = new MessageEmbed()
      ...
      .setDescription(
        'Sunucuda ayarlanmış bir anime kanalı bulamadım. Yeni bir **🔥┃anime・öneri** kanalı oluşturulmasını ve **varsayılan 500 animenin** havuza eklenmesini ister misiniz?',
      )
      .setColor('#5865F2');
    ...
    collector.on('collect', async (interaction) => {
      if (interaction.customId === 'setup_anime_yes') {
        try {
          const newChannel = await message.guild.channels.create(
            '🔥┃anime・öneri',
            {
              type: 'GUILD_TEXT',
              topic:
                'Lütfen bu kanala sadece önermek istediğiniz anime isimlerini yazınız!',
          ...
          console.error(err);
          await interaction.update({
            content: 'Kanal oluşturulurken bir hata meydana geldi!',
            embeds: [],
            components: [],
          });
        }
        ...
        await interaction.update({
          content: 'Kurulum işlemi iptal edildi.',
          embeds: [],
          components: [],
        });
      }
  ...
  try {
    ...
    const availableAnimes = animePool.filter(
      (anime) => !userHistory.includes(anime.toLowerCase()),
    );
    ...
    const randomAnime =
      availableAnimes[Math.floor(Math.random() * availableAnimes.length)];
    ...
    const embed = new MessageEmbed()
      ...
      .setColor('#00FFAA')
      .setFooter({
        text: `${client.user.username} | Havuzda kalan izlemediğin anime: ${availableAnimes.length - 1}`,
      })
      .setTimestamp();
    ...
    console.error(error);
    return message.channel.send(
      `${emojis.bot.error} | Öneri yapılırken teknik bir hata oluştu~ 😢`,
    );
  }
};
